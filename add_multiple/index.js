const places = (array_of_strings = [
  // CHANGE THIS
  "keyword 1",
  "Eg. Restaurant Magicifico",
  "keyword 3",
  // etc…
]);
...
// CHANGE THIS: Replace YOUR_GRAPHQL_ENDPOINT_URL with your GraphQL endpoint and YOUR_HASURA_ADMIN_SECRET with your Hasura admin secret
...
fetchAndInsertPlaces(places).catch(console.error);

async function fetchAndInsertPlaces(placeIdsOrNames) {
  for (const placeIdOrName of placeIdsOrNames) {
    console.log(`Processing: ${placeIdOrName}`);

    // CHANGE THIS: Replace YOUR_GOOGLE_MAPS_API_KEY with your Google Maps API Key
...
async function insertPlaceWithReviewsAndPhotos(place) {
  // CHANGE BELOW: set appropriate user ID
  const mutation = gql`
